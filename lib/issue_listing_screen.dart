...
class IssueListingScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
        shrinkWrap: true,
        itemCount: _wastePin?.length ?? 0,
        itemBuilder: (BuildContext context, int index) {
          return Column(
            children: [
              ListTile(
                title: Text(
                  "Lat:${_wastePin.elementAt(index).location.latitude}, Long:${_wastePin.elementAt(index).location.latitude}",
                  style: TextStyle(
                ...
                onTap: () {
                  ...
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (c) =>
                          IssueListingDetailScreen(_wastePin[index]),
                    ),
                  );
                },
              ),
